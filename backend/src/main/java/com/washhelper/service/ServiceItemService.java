...
public class ServiceItemService {
    ...
    public PageResponse<Map<String, Object>> search(String category, String keyword, Boolean enabled, int page, int pageSize) {
        int normalizedPage = Math.max(page, 1);
        Pageable pageable = PageRequest.of(normalizedPage - 1, pageSize);
        Page<ServiceItem> servicePage = serviceItemRepository.search(emptyToNull(category), emptyToNull(keyword), enabled, pageable);
    ...
    @Transactional
    public ServiceItem create(Map<String, Object> request) {
    ...
    private void apply(ServiceItem item, Map<String, Object> request) {
        if (request.containsKey("name")) {
            item.setName(asString(request.get("name")));
        }
        if (request.containsKey("category")) {
            item.setCategory(asString(request.get("category")));
        }
        if (request.containsKey("price")) {
            item.setPrice(toBigDecimal(request.get("price")));
        }
        if (request.containsKey("duration")) {
            item.setEtaText(asString(request.get("duration")));
        }
        if (request.containsKey("etaText")) {
            item.setEtaText(asString(request.get("etaText")));
        }
        if (request.containsKey("notes")) {
            item.setNotes(asString(request.get("notes")));
        }
        if (request.containsKey("enabled")) {
            item.setActive(toBoolean(request.get("enabled")));
        }
        if (request.containsKey("active")) {
            item.setActive(toBoolean(request.get("active")));
        }
    }
